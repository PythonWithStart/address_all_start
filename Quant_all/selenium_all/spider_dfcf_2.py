...
def get_page_data(page=1):
    # http://36.push2.eastmoney.com/api/qt/clist/get
    url = f"http://61.129.249.233:18665/api/qt/clist/get?cb=&pn={page}&pz=50&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f243&fs=b:MK0354&fields=f1,f152,f2,f3,f12,f13,f14,f227,f228,f229,f229,f230,f231,f232,f233,f234,f235,f236,f237,f238,f239,f240,f241,f242,f26,f243,f116&_=1611665316901"
...
def load_data_excel():
    ...
    f = open(f"funcode_{time_str}.csv", "a", encoding="utf-8")
    fn = open(f"funcode_{time_str}_next.csv", "a", encoding="utf-8")
    ...
    for index, row in enumerate(all_vals):
        if index == 0: 
            f.write("\t".join(row)+"\n")
            fn.write("\t".join(['convertible_bond_code', "stock_code"])+"\n")
            continue
        ...
        # 获取 股票详细数据
        stock_code = row[8]
        f.write("\t".join([str(_row) for _row in row])+"\n")
        fn.write("\t".join([convertible_bond_code, stock_code])+"\n")

        for i in range(2, 9, 1):
            # 获取数据
            _, nall_vals = get_page_data(i)
            for crow in nall_vals:
                crow = filter_data(titles, crow)
                # 获取 可转债详细数据
                convertible_bond_code = crow[2]
                # 获取 股票详细数据
                stock_code = crow[8]
                f.write("\t".join([str(_row) for _row in crow])+"\n")
                fn.write("\t".join([convertible_bond_code, stock_code]) + "\n")
    f.close()
    fn.close()
