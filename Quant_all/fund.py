...
class fundSpider(object):
    ...
    def write_headers(self, path, item):  # 写表头
        with open(path, 'w', newline='', encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(item)

    def write_data(self, path, item):  # 写入数据
    ...
    def listPage(self):  # 列表页
        title = ["基金代码", "基金简称", "日期", "单位净值", "累计净值", "日增长率", "近1周", "近1月", "近3月", "近6月", "近1年", "近2年", "近3年", "今年来", "成立以来", "手续费"]
        self.write_headers(self.listPagePath, title)
        ed = str(time.strftime("%Y-%m-%d"))  # 今天的时间
        for pi in range(1, 123):
            url = "http://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc=zzf&st=desc&sd={}&ed={}&qdii=&tabSubtype=,,,,,&pi={}&pn=50&dx=1".format(ed, ed, str(pi))
            ...
            for data in data_list:
                ...
                for i in range(0, 3):
                    item_list.pop()
                print(pi, item_list)
                self.write_data(self.listPagePath, item_list)

    def history(self):  # 历史净值明细
        self.write_headers(self.historyPath, ["基金代码", "基金简称", "日期", "单位净值", "累计净值", "日增长率"])
        ...
        for source in source_list:
            ...
            for data in data_list:
                ...
                if data == "FSRQ":
                    ...
                    print(y, item)
                    self.write_data(self.historyPath, item)

    def industry(self):  # 行业
        self.write_headers(self.industryPath, ["基金代码", "基金简称", "行业类别", "占净值比例", "市值（万元）"])
        df = pd.read_csv(self.listPagePath, encoding="utf-8", dtype={"基金代码": str})
        source_list = df.values.tolist()
        y = 0
        for source in source_list:
            ...
            for data in HYMC:
                ...
                print(y, item)
                self.write_data(self.industryPath, item)
                num += 1
